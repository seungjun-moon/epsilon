...
def load_pixie_smplx(actions_file=''):
...
class Visualizer(Trainer):
    ...
    def capture(self, savefolder, saveImages=False, video_type='mp4', fps=10):
        ...
        # load data
        try:
            self.train_dataset = build_datasets.build_train(self.cfg.dataset, mode='test')
            print('Capture on test dataset.')
            mode = 'test'
        except:
    ...
    def novel_view(self, savefolder, frame_id=0, saveImages=False, video_type='mp4', fps=10):
        ...
        if 'nerf' in self.cfg.exp_name: #sj
            print('no mesh!')
        ...
        for batch_nb, batch in enumerate(tqdm(self.train_dataloader)):
            ...
            for yaw in tqdm(yaws):
                ...
                # self.model.cfg.use_mesh = False #sj
                opdict = self.model(batch, train=False)
                # visualization
                visdict['render'] = opdict['nerf_image']
                visdict['render_hybrid'] = opdict['nerf_hybrid_image']
                savepath = os.path.join(savefolder, f'{self.cfg.exp_name}_f{frame_id}_{yaw:03}.jpg')
                grid_image = util.visualize_grid(visdict, savepath, return_gird=True, size=512, print_key=True)
                image_list.append(grid_image)
                print(f'saving to {savepath}')
                if saveImages:
        ...
        for batch_nb, batch in enumerate(tqdm(self.train_dataloader)):
            ...
            if self.cfg.use_nerf:
                ...
                for i in tqdm(range(0, xyz.shape[1], chunk)):
                    ...
                    _, sigma_chunk = self.model.query_canonical_space(xyz_chunk, dir_chunk, use_fine=self.model.cfg.use_fine)
        ...
        for i in tqdm(range(full_pose.shape[0])):
            ...
            if self.model.cfg.use_fine:
                visdict = {
                    ...
                    'render_hybrid': opdict['nerf_fine_hybrid_image'],
                    # 'nerf_fine_mask': opdict['nerf_fine_mask'],
                }
            else:
                visdict = {
                    'render': opdict['nerf_image'],
                    'render_hybrid': opdict['nerf_hybrid_image'],
                    # 'shape': opdict['mesh_image'],
                    # 'nerf_mask': opdict['nerf_mask'],
                    # 'nerf_depth': opdict['nerf_depth_vis'],
                    # 'depth_image': opdict['depth_image_vis'],
                    # 'vis_image': opdict['vis_image_vis']
                }
